...
function! s:getFiletype(type, config)
  ...
  if empty(type) || !has_key(config, type)
    let type = &filetype
...
func! Beautifier(...)
  ...
  if executable(engine)
    let program = engine." ".fnameescape(s:plugin_Root_directory."/beautify.min.js")
    let params = "--js_arguments ".tmp_file_Beautifier_arg." ".opts_Beautifier_arg." ".path_Beautifier_arg
    let result = system(program . " " . params)
  else
